...
import {
    ...
    getMasterTokenKey,
    MasterTokenManager,
} from './masterTokenState'
import { MasterTokenContext } from './masterTokenContext'
import { authClient } from '..'

// Dynamically load the client-only provider to avoid importing React hooks
// on the server where they are not allowed. The .client file uses "use client".
export const MasterTokenProvider = dynamic(
    () => import('./masterTokenProvider.client').then((m) => m.MasterTokenProvider),
    { ssr: false }
)

// On the server this hook should not be used. Consumers should import the
// client-side hook from the client provider file. Provide a defensive stub
// to make accidental server-side usage fail fast with a clear message.
export const useMasterToken = () => {
    throw new Error(
        'useMasterToken is a client-side hook. Import it from the client provider or use the dynamically loaded MasterTokenProvider on the client.'
    )
}
        ...
        getActions: () => {
            async function masterTokenSignOut(
                ...
                if (
                    ...
                    console.log('MasterTokenManager.state', MasterTokenManager.state)

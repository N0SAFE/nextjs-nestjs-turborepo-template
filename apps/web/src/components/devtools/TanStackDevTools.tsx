...
const RoutesPluginComponent = () => {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="border rounded-lg p-4">
        <h3 className="text-lg font-semibold mb-2">All Routes</h3>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            <div>
              ...
              <span className="ml-2">{new Date(bundleInfo.buildTime).toLocaleString()}</span>
            </div>
            <div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <span className="ml-2">{Math.round(envInfo.memoryUsage.used / 1024 / 1024)} MB</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="ml-2">{Math.round(envInfo.uptime / 60)} minutes</span>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-2">
            <div>
              ...
              <span className="ml-2 font-mono text-xs">
                {authConfig.databaseUrl !== 'Not configured' ? '***' + authConfig.databaseUrl.slice(-10) : 'Not configured'}
              </span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="ml-2">
                {authConfig.secret === 'configured' ? '*** (configured)' : 'Not configured'}
              </span>
            </div>
            <div>
              <span className="text-sm font-medium">Trust Host:</span>
              <span className="ml-2">{authConfig.trustHost ? 'Yes' : 'No'}</span>
            </div>
          </div>
        ) : (
  ...
  const fetchLogStats = async () => {
    ...
    try {
      ...
      const mockLogs = [
        ...
        { level: 'warning', message: 'Deprecated API usage detected', timestamp: new Date(Date.now() - 60000).toISOString() },
        { level: 'error', message: 'Failed to connect to database', timestamp: new Date(Date.now() - 120000).toISOString() },
        { level: 'info', message: 'User authenticated successfully', timestamp: new Date(Date.now() - 180000).toISOString() },
      ]
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <span className="ml-2 text-red-600">{logStats.errorLogs}</span>
            </div>
            <div>
              <span className="text-sm font-medium">Warning Logs:</span>
              <span className="ml-2 text-yellow-600">{logStats.warningLogs}</span>
            </div>
            <div>
              <span className="text-sm font-medium">Info Logs:</span>
              <span className="ml-2 text-blue-600">{logStats.infoLogs}</span>
            </div>
          </div>
        ) : (

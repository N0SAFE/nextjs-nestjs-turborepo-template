...
describe('useSafeQueryStatesFromZod Hook Tests', () => {
  ...
  describe('Options Handling', () => {
    it('should pass through nuqs options excluding delay and resetKeys', () => {
      ...
      const firstCall = mockUseQueryStates.mock.calls[0]
...
describe('useSafeQueryStatesFromZod - Helper Functions', () => {
  describe('Schema Default Generation', () => {
    ...
    it('should handle native enum types', () => {
      ...
      const schema = z.object({
        enumField: z.enum([TestEnum.VALUE_A, TestEnum.VALUE_B]).default(TestEnum.VALUE_A)
      })
  ...
  describe('Parser Type Handling', () => {
    ...
    it('should handle special parser cases to increase coverage', () => {
      const complexSchema = z.object({
        ...
        // Native enum
        nativeEnumField: z.enum(['a', 'b']).default('a'),
        ...
        refinedEmail: z.email().default('test@example.com')
      })
  ...
  describe('Additional Parser Coverage', () => {
    ...
    it('should handle parser edge cases for specific coverage', () => {
      ...
      const edgeCaseSchema = z.object({
        ...
        emptyEnum: z.enum(['' as const]).optional(),
        
        // Test native enum without values to reach break - using empty enum instead
        emptyNativeEnum: z.enum(['' as const]).optional(),
    ...
    it('should test native enum value extraction edge cases', () => {
      // Test native enum values extraction - using const object instead of TS enum
      const TestEnum = {
        StringValue: 'string',
        NumberValue: 'number' // Use string to avoid mixed enum issue
      } as const

      const nativeEnumEdgeSchema = z.object({
        enumField: z.enum([TestEnum.StringValue, TestEnum.NumberValue]),
        enumWithDefault: z.enum([TestEnum.StringValue, TestEnum.NumberValue]).default(TestEnum.StringValue)
      })
    ...
    it('should handle enum parsing edge cases', () => {
      ...
      const TestEnum = { VALUE1: 'value1', VALUE2: 'value2' } as const
      
      const enumSchema = z.object({
        nativeEnum: z.enum([TestEnum.VALUE1, TestEnum.VALUE2]),
        zodEnum: z.enum(['a', 'b'])
      })

...
export class UserRepository {
  ...
  >(
    ...
    | (Omit<NonNullable<T>, "createdAt" | "updatedAt"> & {
        createdAt: string;
        updatedAt: string;
      })
    | null;
      ...
      .filter({
        ...
        name: (entry) => {
          switch (entry.operator) {
            case "eq":
              return entry.common.eq(user.name);
            case "like":
              return entry.common.like(user.name);
            case "ilike":
              return entry.common.ilike(user.name);
          }
        },
        email: (entry) => {
          switch (entry.operator) {
            case "eq":
              return entry.common.eq(user.email);
            case "like":
              return entry.common.like(user.email);
            case "ilike":
              return entry.common.ilike(user.email);
          }
        },
        ...
        createdAt: ({ operator, value }) => {
          switch (operator) {
            case "gt":
              return gt(user.createdAt, new Date(value));
            case "gte":
              return gte(user.createdAt, new Date(value));
            case "lt":
              return lt(user.createdAt, new Date(value));
            case "lte":
              return lte(user.createdAt, new Date(value));
            case "between": {
              const [from, to] = value;
              return and(

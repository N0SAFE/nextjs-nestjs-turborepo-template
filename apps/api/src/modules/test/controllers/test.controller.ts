...
import {
  requireAuth,
  publicAccess,
} from "@/core/modules/auth/orpc/middlewares";
...
const testContracts = oc.prefix("/test/orpc").router({
  ...
  adminDynamicPermissionEndpoint: oc
    .route({
      method: "POST",
  ...
  orgMembershipStaticEndpoint: oc
    .route({
      method: "GET",
  ...
  orgMembershipDynamicEndpoint: oc
    .route({
      method: "GET",
...
export class TestController {
  ...
  adminPermissionNestJS(@Session() session: UserSession) {
    return {
      message:
        "Admin permission access (see ORPC patterns for AuthService usage)",
  ...
  @ApiResponse({
    status: 403,
    description: "Forbidden - no organization access",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Organization owner access granted",
  })
  @ApiResponse({
    status: 403,
  ...
  organizationOwnerNestJS(
    ...
    return {
      message:
        "Organization owner access (see ORPC patterns for AuthService usage)",
      approach:
        "authService.guards.org.isOrganizationOwner(ctx => ctx.params?.organizationId)",
      userId: session.user.id,
      organizationId,
    };
  }
  ...
  compositeAdminNestJS(
    ...
    return {
      message:
        "Composite admin access (see ORPC patterns for AuthService usage)",
    ...
    return (
      implement(testContracts.adminRoleEndpoint)
        ...
        .handler(({ context }) => {
          ...
          return {
            message:
              "Admin role access granted via authService.middleware.admin.hasRole()",
  ...
  adminPermissionOrpc() {
    return (
      implement(testContracts.adminPermissionEndpoint)
        ...
        .use(
          this.authService.middleware.admin.hasPermission({ user: ["list"] }),
        )
        .handler(({ context }) => {
          const auth = assertAuthenticated(context.auth);
          return {
            message:
              "Admin permission access granted via authService.middleware.admin.hasPermission()",
            userId: auth.user.id,
            userRole: auth.user.role ?? "unknown",
          };
        })
    ...
    return (
      implement(testContracts.adminDynamicPermissionEndpoint)
        ...
        .handler(({ context, input }) => {
          ...
          return {
            message:
              "Dynamic admin permission granted via authService.middleware.admin.hasPermission(resolver)",
    ...
    return (
      implement(testContracts.orgMembershipStaticEndpoint)
        ...
        .handler(({ context }) => {
          ...
          return {
            message:
              "Organization membership granted via authService.middleware.org.isMemberOf('static')",
            userId: auth.user.id,
          };
        })
    );
    ...
    return (
      implement(testContracts.orgMembershipDynamicEndpoint)
        ...
        .handler(({ context, input }) => {
          ...
          return {
            message:
              "Organization membership granted via authService.middleware.org.isMemberOf(input => input.organizationId)",
  ...
  orgRoleOrpc() {
    ...
    return (
      implement(testContracts.orgRoleEndpoint)
        ...
        .use(
          this.authService.middleware.org.hasOrganizationRole<Input>(
            (ctx) => ctx.input.organizationId,
        ...
        .handler(({ context, input }) => {
          ...
          return {
            message:
              "Organization role access granted via authService.middleware.org.hasOrganizationRole()",
            organizationId: input.organizationId,
            userId: auth.user.id,
          };
        })
    ...
    return (
      implement(testContracts.orgOwnerEndpoint)
        ...
        .handler(({ context, input }) => {
          ...
          return {
            message:
              "Organization owner access granted via authService.middleware.org.isOrganizationOwner.forInput()",
            organizationId: input.organizationId,
            userId: auth.user.id,
          };
        })
  ...
  orgPermissionOrpc() {
    return (
      implement(testContracts.orgPermissionEndpoint)
        ...
        .use(
          this.authService.middleware.org.hasOrganizationPermission({
            orgMember: ["list"],
          }),
        )
        .handler(({ context }) => {
          ...
          return {
            message:
              "Organization permission granted via authService.middleware.org.hasOrganizationPermission()",
            userId: auth.user.id,
          };
        })
    );
    ...
    return (
      implement(testContracts.compositeAdminEndpoint)
        ...
        .handler(({ context }) => {
          ...
          return {
            message:
              "Composite admin access granted via authService.middleware.admin.composite()",
            userId: auth.user.id,
            userRole: auth.user.role ?? "unknown",
          };
        })
  ...
  contextAccessOrpc() {
    ...
    return (
      implement(testContracts.contextAccessEndpoint)
        ...
        .use(
          this.authService.middleware.org.isMemberOf<Input>(
        ...
        .handler(({ context, input }) => {
          ...
          return {
            message:
              "Type-safe context access demonstrated via <TInput, TContext> generics",
            organizationId: input.organizationId,
            userId: auth.user.id,
            userRole: auth.user.role ?? "unknown",
          };

...
export class CliAuthService {
  ...
  async getAuthenticatedHeaders(options?: {
    ...
    const enableMasterToken = this.configService.get<boolean>(
      "ENABLE_MASTER_TOKEN",
    );
    const defaultAdminEmail = this.configService.get<string>(
      "DEFAULT_ADMIN_EMAIL",
    );
    const envAdminPassword = this.configService.get<string>(
    ...
    if (devAuthKey && enableMasterToken) {
      ...
      try {
        ...
        return {
          headers: authHeaders,
          cleanup: async () => {
            /* No cleanup needed for master token */
          },
          method: "master-token",
        };
      } catch (error) {
        if (verbose) {
          console.log(
            `   ⚠️ Master token auth failed: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }
    }
    ...
    if (defaultAdminEmail && adminPassword) {
      ...
      try {
        const session = await this.signInAsUser(
          defaultAdminEmail,
          adminPassword,
        );
        if (session) {
        ...
        if (verbose) {
          console.log(
            `   ⚠️ Default admin auth failed: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }
    }
  ...
  async signInAsUser(
    email: string,
    password: string,
  ): Promise<{ headers: Headers; userId: string } | null> {
    try {
  ...
  async createTempUser(options?: {
    ...
    if (verbose)
      console.log(`   ✅ Created temporary CLI user (ID: ${userId})`);
  ...
  async ensureDevAuthUser(): Promise<void> {
    ...
    if (!devAuthEmail) {
      console.log(
        "ℹ️  DEFAULT_ADMIN_EMAIL not configured, skipping dev auth user creation",
      );
      return;
    }
    ...
    if (existingUser.length > 0 && existingUser[0]) {
      console.log(
        `✅ Dev auth user already exists: ${devAuthEmail} (ID: ${existingUser[0].id})`,
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `   This user can be impersonated using DEV_AUTH_KEY in development mode`,
      );
    } catch (error) {
      // If user creation fails, log and continue
      console.warn(
        `⚠️  Could not create dev auth user: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async ensureDefaultAdminUser(): Promise<string | null> {
    ...
    const envPassword = this.configService.get<string>(
      "DEFAULT_ADMIN_PASSWORD",
    );

    if (!defaultAdminEmail) {
      console.log(
        "ℹ️  DEFAULT_ADMIN_EMAIL not configured, skipping default admin user creation",
      );
      return null;
    }
    ...
    if (existingUser.length > 0 && existingUser[0]) {
      console.log(
        `✅ Default admin user already exists: ${defaultAdminEmail} (ID: ${existingUser[0].id})`,
      );
    ...
    try {
      ...
      if (!envPassword) {
        ...
        console.log(
          `   You can also set DEFAULT_ADMIN_PASSWORD env var to use a specific password.`,
        );
        console.log(`${separator}\n`);
      } else {
        console.log(
          `✅ Created default admin user: ${defaultAdminEmail} (ID: ${userId})`,
        );
      }
      ...
      console.warn(
        `⚠️  Could not create default admin user: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }

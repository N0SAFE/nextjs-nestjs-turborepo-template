 ...
 */
export type OrganizationPluginWrapper<
  TAuth extends ApiMethodsWithOrganizationPlugin<OrganizationBuilder> =
    ApiMethodsWithOrganizationPlugin<OrganizationBuilder>,
> = OrganizationsPermissionsPlugin<OrganizationBuilder, TAuth>;
...
>(auth: TAuth) {
  return new PluginWrapperRegistry<TAuth>(auth)
    ...
    .register("organization", createOrganizationWrapper);
}
...
export type InferPluginsFromRegistry<
  TPluginRegistry extends PluginWrapperRegistry<any, any>,
> = ReturnType<TPluginRegistry["getAll"]>;
...
export function createAdminWrapper<
  TAuth extends ApiMethodsWithAdminPlugin<PlatformBuilder>,
>(auth: TAuth, headers: Headers, session?: InferSessionFromAuth<TAuth> | null) {
...
>(auth: TAuth, registry: ReturnType<typeof createPluginRegistry<TAuth>>) {
  return {
    admin: new AdminMiddlewareDefinition<typeof platformBuilder, TAuth>(
      (context) => registry.create("admin", context.headers, context.session),
    ),
    organization: new OrganizationMiddlewareDefinition<
      typeof organizationBuilder,
      TAuth
    >((context) =>
      registry.create("organization", context.headers, context.session),
    ),
  };
}

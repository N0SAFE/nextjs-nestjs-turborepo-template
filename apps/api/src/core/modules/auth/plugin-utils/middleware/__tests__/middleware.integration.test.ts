...
interface MockAdminPlugin {
  ...
  getAuth: Mock<
    () => {
      api: {
        getSession: (opts: { headers: Headers }) => Promise<MockSession | null>;
      };
    }
  >;
  checkPermission: Mock<
    (perms: Record<string, readonly string[]>) => Promise<boolean>
  >;
  checkRolePermission: Mock<
    (role: string, perms: Record<string, readonly string[]>) => Promise<boolean>
  >;
  assertCheckPermission: Mock<
    (
      perms: Record<string, readonly string[]>,
      message?: string,
    ) => Promise<void>
  >;
  assertCheckRole: Mock<
    (roles: readonly string[], message?: string) => Promise<void>
  >;
}
...
interface MockOrgPlugin {
  ...
  getOrganization: Mock<
    (orgId: string) => Promise<{ id: string; name: string; creatorId?: string }>
  >;
}
 ...
 */
function createMockAdminPlugin(
...
describe("AdminMiddlewareDefinition", () => {
  ...
  describe("Check Creation", () => {
    it("should create hasPermission check", () => {
      ...
      expect(check.description).toBe(
        "Checks if user has the specified permissions",
      );
    });
  ...
  describe("hasPermission Check Execution", () => {
    ...
    it("should return meaningful error message", () => {
      const check = middleware.hasPermission({ user: ["create"] });
      expect(check.getErrorMessage()).toBe(
...
describe("OrganizationMiddlewareDefinition", () => {
  ...
  describe("Check Creation", () => {
    ...
    it("should create hasOrganizationRole check", () => {
      const check = middleware.hasOrganizationRole("org-123", [
        "admin",
        "owner",
      ]);
  ...
  describe("hasOrganizationPermission Check Execution", () => {
    ...
    it("should throw when user lacks organization permission", async () => {
      ...
      await expect(check.check(context)).rejects.toThrow(
  ...
  describe("Dynamic Organization ID Resolution", () => {
    it("should resolve organization ID from context", async () => {
      ...
      const check = middleware.isMemberOf(
...
import { createAdminMiddleware, createOrganizationMiddleware } from "../index";
...
describe("Convenience Factory Functions", () => {
  ...
  describe("createOrganizationMiddleware", () => {
    it("should create OrganizationMiddlewareDefinition from plugin", () => {
      ...
      const middleware = createOrganizationMiddleware(
        plugin as unknown as OrganizationsPermissionsPlugin<
          MockPermissionBuilder,
          any
        >,
      );
  ...
  describe("Factory function usage patterns", () => {
    ...
    it("should work in typical workflow with ORPC middleware creation", () => {
      ...
      const orpcMiddleware = createOrpcMiddleware(
    ...
    it("should work with composite guards from multiple definitions", () => {
      ...
      const adminMiddleware = createAdminMiddleware(
        adminPlugin as unknown as AdminPermissionsPlugin<
          MockPermissionBuilder,
          any
        >,
      );
      const orgMiddleware = createOrganizationMiddleware(
        orgPlugin as unknown as OrganizationsPermissionsPlugin<
          MockPermissionBuilder,
          any
        >,
      );

 ...
 */
type AdminMiddlewareDefinition = ReturnType<
  typeof createPluginMiddlewares
>["admin"];

/**
 * Type for the organization middleware definition returned by createPluginMiddlewares().organization
 */
type OrganizationMiddlewareDefinition = ReturnType<
  typeof createPluginMiddlewares
>["organization"];
 ...
 */
type WidenPermission<T> = T extends StrictAdminPermissions
  ? AdminPermissions
  : T extends StrictOrgPermissions
    ? OrgPermissions
    : T;
...
type WidenArg<T, TInput, TContext = unknown> =
  ...
  | ((
      ctx: OrpcMiddlewareOptionsContext<TInput, TContext>,
    ) => WidenPermission<Exclude<T, (...args: never[]) => unknown>>); // Widen the resolver part

/**
 * Extract the first param type from a method for forInput() return type.
 */
type ExtractFirstParam<T> = T extends (
  arg: infer P,
  ...rest: never[]
) => unknown
  ? P
 ...
 */
type UnwrapValueOrResolver<T> = T extends ((ctx: never) => unknown) | infer V
  ? V
  : T;
 ...
 */
type WidenedProxyMethod<TMethod> = TMethod extends (
  ...args: infer TArgs
) => infer TReturn
  ? {
...
class AdminChecksBuilder {
  constructor(
    private readonly getMiddlewares: () => ReturnType<
      typeof createPluginMiddlewares
    >,
  ) {}
  ...
  hasPermission(permissions: AdminPermissions): MiddlewareCheck {
    return this.getMiddlewares().admin.hasPermission(
      permissions as StrictAdminPermissions,
    );
  }
  ...
  hasPermissionByRole(
    ...
    return this.getMiddlewares().admin.hasPermissionByRole(
      role,
      permissions as StrictAdminPermissions,
    );
  }
...
class OrganizationChecksBuilder {
  ...
  hasOrganizationRole(
    ...
    roles: readonly OrgRoles[],
  ): MiddlewareCheck {
    return this.getMiddlewares().organization.hasOrganizationRole(
      organizationId,
      roles,
    );
  }
  ...
  isOrganizationOwner(
    ...
    return this.getMiddlewares().organization.isOrganizationOwner(
      organizationId,
    );
  }

  hasOrganizationPermission(permissions: OrgPermissions): MiddlewareCheck {
    return this.getMiddlewares().organization.hasOrganizationPermission(
      permissions as StrictOrgPermissions,
    );
  }
}
...
export class AuthCoreService<T extends AuthWithPlugins = Auth> {
   ...
   * ```
   */
  plugin<K extends keyof PluginRegistry>(
    name: K,
    headers: Headers,
  ): PluginRegistry[K] {
   ...
   * The assertion is required due to TypeScript variance restrictions.
   */
  getMiddlewares(): ReturnType<
    typeof createPluginMiddlewares<AuthWithPlugins>
  > {
    const authConfig = this.options.auth;
    let cached = registryCache.get(authConfig);
      ...
      .middleware(async (opts) => {
        ...
        const session = await auth.api.getSession({
          headers: normalizeHeaders(
            opts.context.request.headers as RequestHeaders,
          ),
        });

...
export class AuthModule implements NestModule, OnModuleInit {
  ...
  configure(consumer: MiddlewareConsumer): void {
    const trustedOrigins =
      "trustedOrigins" in this.auth.options ? this.auth.options : [];
    ...
    let basePath =
      "basePath" in this.auth.options &&
      typeof this.auth.options.basePath === "string"
        ? this.auth.options.basePath
        : "/api/auth";
  ...
  private setupHooks(
    ...
    if (
      ...
      typeof this.auth.options.hooks !== "object" ||
      this.auth.options.hooks === null
    )
      return;

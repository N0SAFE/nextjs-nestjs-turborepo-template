...
export class LoggerMiddleware implements NestMiddleware {
  ...
  private readonly logSafelist = ["/health", "/metrics", "/favicon.ico"];
  ...
  use(req: Request, res: Response, next: () => void): void {
    ...
    res.on("close", () => {
      ...
      this.logger.debug(
        `[${hostname}] "${method} ${greenPath}" ${String(statusCode)} ${statusMessage} ${String(contentLength)} "${referer}" "${userAgent}" "${String(ip)}"`,
      );
